...
```{r}
#Cleaning up a team name lookup table
teamNames <- teams_F %>% select(c("tmID", "name")) %>% 
             unique() %>% filter(name!="Chicago Black Hawks")

#Cleaning up a coach name lookup table
coaches_names <- masters %>% select("coachID", "firstName", "lastName") %>% 
                 filter(!is.na(coachID))

#Join coach data with coach names
topCoaches<- coaches %>% group_by(coachID, tmID) %>%  tally() %>% arrange(desc(n)) %>% 
             inner_join(., coaches_names, by="coachID") %>% 
             mutate(nameC = paste(firstName, lastName, sep=" ")) %>% head(10)

#Joing team name with tibble of top coaches
